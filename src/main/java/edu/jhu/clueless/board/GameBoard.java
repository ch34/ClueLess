		...
		for (int x = 0; x < 5; x++) {
			for (int y = 0; y < 5; y++) {
				...
				if (x % 2 == 0 && y % 2 == 0) {
					...
					switch (xy) {
						case "0,0": room = new GameSquare(Room.STUDY, Room.KITCHEN); break;
						case "0,2": room = new GameSquare(Room.LIBRARY); break;
						case "0,4": room = new GameSquare(Room.CONSERVATORY, Room.LOUNGE); break;
						case "2,0": room = new GameSquare(Room.HALL); break;
						case "2,2": room = new GameSquare(Room.BILLIARD_ROOM); break;
						case "2,4": room = new GameSquare(Room.BALLROOM); break;
						case "4,0": room = new GameSquare(Room.LOUNGE, Room.CONSERVATORY); break;
						case "4,2": room = new GameSquare(Room.DINING_ROOM); break;
						case "4,4": room = new GameSquare(Room.KITCHEN, Room.STUDY);
					}
